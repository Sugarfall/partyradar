  ...
  if (!dbUser) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#07071a' }}>
      <div className="text-center space-y-3">
        <Music size={36} style={{ color: 'rgba(0,229,255,0.2)', margin: '0 auto' }} />
        ...
        <a href="/login" className="inline-block px-6 py-2.5 rounded-xl text-xs font-black"
          style={{ background: 'rgba(0,229,255,0.1)', border: '1px solid rgba(0,229,255,0.3)', color: '#00e5ff' }}>LOG IN</a>
      </div>
    </div>
  )
  ...
  if (submitted) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#07071a' }}>
      <div className="text-center space-y-4 px-6">
        ...
        <p className="text-xl font-black" style={{ color: '#00e5ff' }}>Request Sent!</p>
        ...
        <button onClick={() => router.back()} className="px-6 py-2.5 rounded-xl text-xs font-black"
          style={{ background: 'rgba(0,229,255,0.08)', border: '1px solid rgba(0,229,255,0.2)', color: '#00e5ff' }}>GO BACK</button>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: '#07071a' }}>
      <div className="sticky top-0 z-10 px-4 pt-12 pb-4"
        ...
        <p className="text-xs mt-1" style={{ color: 'rgba(0,229,255,0.4)' }}>Costs {CREDIT_COST} reward points</p>
      </div>

      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        {points !== null && (
          <div className="flex items-center gap-2 px-4 py-3 rounded-xl"
            style={{ background: hasEnough ? 'rgba(0,229,255,0.04)' : 'rgba(255,0,110,0.04)', border: `1px solid ${hasEnough ? 'rgba(0,229,255,0.12)' : 'rgba(255,0,110,0.2)'}` }}>
            <Zap size={14} style={{ color: hasEnough ? '#00e5ff' : '#ff006e' }} />
            <p className="text-xs font-bold" style={{ color: hasEnough ? '#00e5ff' : '#ff006e' }}>
