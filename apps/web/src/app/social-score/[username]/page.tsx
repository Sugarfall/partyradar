  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: '#07071a' }}>
      <div className="sticky top-0 z-10 px-4 pt-12 pb-4"
        ...
        <p className="text-xs text-center mt-1" style={{ color: 'rgba(0,229,255,0.4)' }}>@{username}</p>
      </div>

      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        {/* Score */}
        <div className="p-6 rounded-2xl text-center" style={{ background: 'rgba(7,7,26,0.8)', border: '1px solid rgba(0,229,255,0.1)' }}>
          <div className="text-5xl font-black mb-1" style={{ color: '#00e5ff' }}>{data?.socialScore ?? 0}</div>
          <p className="text-xs font-bold tracking-widest" style={{ color: 'rgba(0,229,255,0.4)' }}>OUT OF 100</p>
        </div>

        {/* Category breakdown */}
        {data && Object.keys(data.avgByCategory).length > 0 && (
          <div className="p-4 rounded-2xl space-y-3" style={{ background: 'rgba(7,7,26,0.8)', border: '1px solid rgba(0,229,255,0.08)' }}>
        ...
        {showForm && (
          <div className="p-4 rounded-2xl space-y-4" style={{ background: 'rgba(7,7,26,0.9)', border: '1px solid rgba(0,229,255,0.12)' }}>
            <p className="text-[10px] font-black tracking-[0.2em]" style={{ color: 'rgba(0,229,255,0.4)' }}>ANONYMOUS FEEDBACK</p>
              ...
              {CATEGORIES.map(cat => (
                <button key={cat.key} onClick={() => setFbCategory(cat.key)}
                  ...
                  style={fbCategory === cat.key
                    ...
                    : { background: 'rgba(7,7,26,0.6)', border: '1px solid rgba(0,229,255,0.08)', color: 'rgba(224,242,254,0.5)' }}>
                  {cat.emoji} {cat.label}
                </button>
              ))}
            </div>
              ...
              {[1,2,3,4,5].map(n => (
                <button key={n} onClick={() => setFbScore(n)}
                  ...
                  style={n <= fbScore
                    ...
                    : { background: 'rgba(7,7,26,0.6)', border: '1px solid rgba(0,229,255,0.08)', color: 'rgba(224,242,254,0.3)' }}>
                  {n}
                </button>
              ))}
            </div>
        ...
        {data && data.recentFeedback.length > 0 && (
          <div className="space-y-2">
            <p className="text-[10px] font-black tracking-[0.2em] px-1" style={{ color: 'rgba(0,229,255,0.3)' }}>RECENT FEEDBACK</p>
            {data.recentFeedback.map((fb, i) => (
              <div key={i} className="p-3 rounded-xl" style={{ background: 'rgba(7,7,26,0.7)', border: '1px solid rgba(0,229,255,0.06)' }}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-bold uppercase" style={{ color: 'rgba(0,229,255,0.5)' }}>
