...
export default function ProfilePage() {
  ...
  // Fetch medals — auto-seed+check first, then load earned list
  useEffect(() => {
    if (!dbUser) return
    const fetchMedals = () =>
      api.get<{ data: EarnedMedal[] }>(`/medals/user/${dbUser.id}`)
      ...
      >
        ...
        <div className="max-w-xl mx-auto flex items-center gap-5">
          ...
          <div className="flex flex-col items-center shrink-0">
            ...
            {photoError && (
              <p className="text-[9px] font-bold mt-1 text-center max-w-[120px] leading-tight" style={{ color: '#ff006e' }}>
                ⚠ {photoError}
              </p>
            )}
          </div>
