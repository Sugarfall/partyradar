  ...
  if (!dbUser) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#07071a' }}>
      <div className="text-center space-y-3">
        <Gift size={36} style={{ color: 'rgba(0,229,255,0.2)', margin: '0 auto' }} />
        <a href="/login" className="text-sm font-black" style={{ color: '#00e5ff' }}>Log in to get your referral card</a>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: '#07071a' }}>
      <div className="sticky top-0 z-10 px-4 pt-12 pb-4"
        ...
        <p className="text-xs mt-1" style={{ color: 'rgba(0,229,255,0.4)' }}>Earn 5% on every referral purchase</p>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        {!loading && !card && (
          <div className="space-y-4">
            <div className="p-6 rounded-2xl text-center space-y-3" style={{ background: 'rgba(7,7,26,0.8)', border: '1px solid rgba(0,229,255,0.1)' }}>
              <Gift size={40} style={{ color: 'rgba(0,229,255,0.3)', margin: '0 auto' }} />
            ...
            <div className="space-y-2">
              {[['5% commission', 'Earn on tickets, subs, and group chat purchases'],
                ...
                <div key={title} className="flex items-start gap-3 p-3 rounded-xl" style={{ background: 'rgba(7,7,26,0.6)', border: '1px solid rgba(0,229,255,0.06)' }}>
                  <div className="w-1.5 h-1.5 rounded-full mt-2 shrink-0" style={{ background: '#00e5ff' }} />
                  <div>
        ...
        {!loading && card && (
          <>
            ...
            <div className="p-6 rounded-2xl relative overflow-hidden"
              style={{ background: 'linear-gradient(135deg, #07071a 0%, #0f0a2a 100%)', border: '1px solid rgba(0,229,255,0.25)', boxShadow: '0 0 40px rgba(0,229,255,0.06)' }}>
              <div className="absolute top-0 right-0 w-32 h-32 rounded-full opacity-10" style={{ background: 'radial-gradient(circle, #00e5ff, transparent)', transform: 'translate(30%, -30%)' }} />
              <p className="text-[9px] font-black tracking-[0.3em] mb-1" style={{ color: 'rgba(0,229,255,0.4)' }}>PARTYRADAR · REFERRAL CARD</p>
              <p className="text-lg font-black mb-4" style={{ color: '#e0f2fe' }}>{card.displayName}</p>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-2xl font-black tracking-widest" style={{ color: '#00e5ff', fontFamily: 'monospace' }}>{card.code}</p>
                  <p className="text-[10px] mt-1" style={{ color: 'rgba(0,229,255,0.35)' }}>5% REVENUE SHARE</p>
                </div>
                <button onClick={copyCode} className="p-3 rounded-xl"
                  style={{ background: copied ? 'rgba(0,255,136,0.1)' : 'rgba(0,229,255,0.08)', border: `1px solid ${copied ? 'rgba(0,255,136,0.3)' : 'rgba(0,229,255,0.2)'}`, color: copied ? '#00ff88' : '#00e5ff' }}>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={label} className="p-3 rounded-xl text-center" style={{ background: 'rgba(7,7,26,0.8)', border: '1px solid rgba(0,229,255,0.08)' }}>
                  <p className="text-base font-black" style={{ color: '#00e5ff' }}>{value}</p>
                  <p className="text-[9px] font-bold mt-0.5" style={{ color: 'rgba(0,229,255,0.3)' }}>{label}</p>
                </div>
              ))}
            </div>
            ...
            {card.conversions.length > 0 && (
              <div className="space-y-2">
                <p className="text-[10px] font-black tracking-[0.2em] px-1" style={{ color: 'rgba(0,229,255,0.3)' }}>RECENT CONVERSIONS</p>
                {card.conversions.map((c, i) => (
                  <div key={i} className="flex items-center justify-between px-4 py-3 rounded-xl"
                    style={{ background: 'rgba(7,7,26,0.7)', border: '1px solid rgba(0,229,255,0.06)' }}>
                    <div>
            ...
            <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(7,7,26,0.5)', border: '1px solid rgba(0,229,255,0.06)' }}>
              <p className="text-xs" style={{ color: 'rgba(224,242,254,0.3)' }}>
                Share your code <strong style={{ color: 'rgba(0,229,255,0.6)' }}>{card.code}</strong> and earn 5% on every purchase your referrals make on PartyRadar.
              </p>
            </div>
          </>
        )}
